/* Painel: o Resumo (visão de chegada com KPIs, prazos, pipeline e alertas)
   e as Pendências (o que falta em pessoas, artistas, projetos, candidaturas
   e editais — com atalho pra criar tarefa a partir da lacuna). */
import type { ReactNode } from "react";
import { usarCentral } from "../banco/dados";
import { abrirDetalhe } from "../estado/navegacao";
import { abrirEdicao, abrirNovo, type ChaveEntidade } from "../estado/edicao";
import { Shead, eventosAgenda, nomeEquipe, projetoArtistaDe, tituloCurto } from "./comuns";
import { ETAPAS_PIPELINE } from "../tipos";
import { formatarData } from "../util";

export function VisaoResumo() {
  ...
  return (
    <>
      <Shead titulo="Resumo" sub="visão de chegada — tudo se atualiza sozinho conforme você edita" />
      <div className="kpis">
        {kpis.map(([rotulo, n]) => (
...
export function VisaoPendencias() {
  const { painel } = usarCentral();
  ...
  return (
    <>
      <Shead titulo="Pendências — o que falta" sub={sub} />
